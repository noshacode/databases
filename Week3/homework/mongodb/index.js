...
async function createEpisodeExercise(client) {
	...
	const item = await bobRossCollection.insertOne({
		episode: "S09E13",
	...
	console.log(
		`Created season 9 episode 13 and the document got the id ${item.insertedId}`
	);
...
async function findEpisodesExercises(client) {
	...
	const findTitle = await bobRossCollection.findOne(
		{ episode: "S02E02" },
		{ title: true }
	);
	...
	const findEpisode = await bobRossCollection.findOne({ title: "BLACK RIVER" });
	console.log(
		`The season and episode number of the "BLACK RIVER" episode is ${findEpisode.episode}`
	);

	// Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]
	const findCliff = await bobRossCollection.find({ elements: "CLIFF" }, {title: true}).toArray()
	console.log(
		`The episodes that Bob Ross painted a CLIFF  ${findCliff.map(i=> i.title)}`
	);

	// Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
	const findAllEpisode = await bobRossCollection.find({ elements: "CLIFF ",elements:"LIGHTHOUSE"},{title: true}).toArray()
	console.log(
		`The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${findAllEpisode.map(i=> i.title)}`
	);
}
...
async function updateEpisodeExercises(client) {
  ...
  const  blueRidgeFalls = await bobRossCollection.findOneAndUpdate({ title: "BLUE RIDGE FALLERS" }, {$set:{title: "BLUE RIDGE FALLS"}})
  
  console.log(
		`Ran a command to update episode 13 in season 30 and it updated ${blueRidgeFalls.value.episode} episodes`
	);
